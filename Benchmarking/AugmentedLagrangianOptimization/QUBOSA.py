...
def AugmentedLagrangianQuad( Obj,Qcost,Aineq, bineq, Qdemand,Qzcorr,ListQuadraticTerms,fixVariableToZero, fixVariableToOne,lList ,wList,whereToSave  ,maxiter=15):
    '''
    Parameters
    ----------
    Obj : numpy array
   ...
   ListQuadraticTerms : list
    ...
    for k in range(maxiter):
        ...
        #plt.imshow( np.maximum( np.minimum( fullQUBO,10*np.ones(fullQUBO.shape)),-10*np.ones(fullQUBO.shape)))
        #plt.colorbar()
        #plt.savefig(whereToSave+'ImgPlots'+str(k)+'.png', format='png')
        #plt.show()
...
def FixVariableSquare(QuboList, N ,currSol):
    ...
    Parameters
    ----------
    QuboList : List of numpy arrays
        QUBO terms for optimization
    N : int
        Size s. publication
    currSol : numpy array
        all current binary variables

    Returns
    -------
    QlambList : 
        Fix all z variables in the unrestricted general formulation.
