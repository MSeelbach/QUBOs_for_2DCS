...
Perform experiments on smaller literature instances via the Augmented Lagrangian method.
"""
import numpy as np
import csv
...
mypath= 'C:/Users/MarcelSeelbach(MSE)/Documents/Gitlab/magicapp/NewIdeaEmphasizeCost/phd-BMC-1/instances/' #where are the instances stored?
...
for m in range(len(onlyfiles)):
    #We restrict later to smaller problems up to 30 parts   
        ...
        print(onlyfiles[m]+' '+str(np.sum(be)))
        
        if np.sum(be)<=30: #Only try out problems with up to 30 parts
...
for problemIter, problem in enumerate(StoreProblemInformation):
 # if problemIter==3: 
    ...
    def RecursiveCuttingPlan(XYsolution, Zsolution,currentEntry,joinedPieces, doneEntries, xPos ,yPos , cuttingStage, positionInfo=[] , rev=0 ) :
                ...
                Parameters
                ----------
                XYsolution : numpy array
                     x and y variables from the publication
                Zsolution : numpy array
                    z variables from the publication
                currentEntry : int
                    what entry of the vector are we looking at
                joinedPieces : List
                    Which pieces are joined
                doneEntries : List
...
.
    ...
    def setUpLinProgUnr(additFactor=1):
        '''
        (Not used for benchmaring table in publication)
        Set up a linear programm to solve the 2D cutting stock problem 
        
        Output: objective vector c and and in. constraints [c,A,b]
        '''
    ...
    for s in range(multipleTrys):
        ...
        solList, bestSol,EnLin, EnQuad, bestIter = QUBOSA.AugmentedLagrangianQuad(   scipy.sparse.diags(c.toarray(), [0]), Qcost ,Asize, bsize, Qdemand,Qzcorr,ListQuadraticTerms ,fixVariableToZero, fixVariableToOne,lList,wList, whereToSave , maxiter=100)
