...
Carry out the expirements with the 0-1 integer linear programming formulation on the literature problem instances.
"""
import numpy as np
import csv
...
import Highssolver #copy from utilclasses
import random
    ...
    if  onlyfiles[m].startswith("CW")  or onlyfiles[m].startswith("CU") :
        ...
        with open(mypath + onlyfiles[m], newline='') as csvfile:
            ...
            for row in spamreader:
                asList.append(row)
        try:
            L,W = int(asList[0][0]),int(asList[0][1])
...
for problemIter, problem in enumerate(StoreProblemInformation):
    ...
    for s in range(multipleTrys):
        ...
        sol = Highssolver.linprog(c,A, b ,None,None, bounds = bounds ,integrality=[1]*(NrVariables),Inputoptions= { 'disp':True,'presolve':True, 'mip_heuristic_effort':0.1,'time_limit':180*10 },SaveStr=descrStr+'.txt')  #NealSolver.runSimulatedFullyConnectedQUBOSparse(Q[np.ix_(restVariables[:], restVariables[:])])#initial_states =[numpyArrayToNealDict(np.ones(Q.shape[0]))]) #initial_states =[numpyArrayToNealDict(solBestSim)])
    ...
    plt.savefig('PlotsLinprogNoHeur/' + descrStr + '.png')
    end= timeit.timeit()

    pickle.dump([sol, Energy  ,end-start ], open( "PlotsLinprogNoHeur/save"+descrStr+".p", "wb" ) )    
    plt.show()
